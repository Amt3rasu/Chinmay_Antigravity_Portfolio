...
const BORDERLESS_PROJECTS = ['ui-designs', 'cican-navigator'];

const ContentRenderer: React.FC<{ blocks: ContentBlock[], compact?: boolean, projectId?: string }> = ({ blocks, compact = false, projectId }) => {
    if (!blocks) return null;
    const isBorderless = projectId && BORDERLESS_PROJECTS.includes(projectId);


    const renderBlock = (block: ContentBlock, index: number) => {
        switch (block.type) {
            case 'h1':
                const h1Style = isBorderless && !compact ? 'px-8 md:px-14 lg:px-20 max-w-4xl ml-auto !mt-24 !mb-8' : '';
                return <h1 key={index} className={`text-3xl md:text-4xl font-serif font-bold text-foreground ${compact ? 'mt-0 mb-2' : 'mt-12 mb-6'} tracking-tight ${h1Style}`}>{block.content}</h1>;
            case 'h2':
                const h2Style = isBorderless && !compact ? 'px-8 md:px-14 lg:px-20 max-w-4xl ml-auto !mt-16 !mb-6' : '';
                return <h2 key={index} className={`text-2xl font-bold text-foreground ${compact ? 'mt-0 mb-6' : 'mt-10 mb-4'} ${h2Style}`}>{block.content}</h2>;
            case 'h3':
                const h3Style = isBorderless && !compact ? 'px-8 md:px-14 lg:px-20 max-w-4xl ml-auto !mt-12 !mb-4' : '';
                return <h3 key={index} className={`text-xl font-bold text-foreground ${compact ? 'mt-0 mb-3' : 'mt-8 mb-3'} ${h3Style}`}>{block.content}</h3>;
            case 'p':
                const pStyle = isBorderless && !compact ? 'px-8 md:px-14 lg:px-20 max-w-3xl ml-auto !mb-12 text-xl' : '';
                return <p key={index} className={`text-lg text-muted-foreground ${compact ? 'mb-2' : 'mb-6'} leading-relaxed ${pStyle}`} dangerouslySetInnerHTML={{ __html: block.content.replace(/\*\*(.*?)\*\*/g, '<strong class="text-foreground">$1</strong>') }}></p>;
            case 'image':
                if (isBorderless) {
                    // For borderless projects, break out of the container to be full width
                    return (
                        <div key={index} className="w-screen relative left-1/2 -translate-x-1/2 px-3 md:px-6 lg:px-8 my-16 bg-background">
                            <img src={block.src} alt={block.alt} className={`w-full max-w-none object-cover rounded-2xl md:rounded-[2rem] shadow-2xl ${block.className || ''}`} />
                        </div>
                    );
                }
                return <img key={index} src={block.src} alt={block.alt} className={`${compact ? 'my-0' : 'my-10'} rounded-2xl shadow-lg w-full border border-border ${block.className || ''}`} />;
            case 'list': return (
            ...
            case 'split_layout': return (
                <div key={index} className={`my-12 grid md:grid-cols-2 gap-8 lg:gap-16 items-center`}>
                    <div className={`${block.reverseOnMobile ? 'md:order-last' : ''}`}><ContentRenderer blocks={block.left} projectId={projectId} /></div>
                    <div className={`${block.reverseOnMobile ? 'md:order-first' : ''}`}><ContentRenderer blocks={block.right} projectId={projectId} /></div>
                </div>
            );
                ...
                <div key={index} className="my-16 grid grid-cols-1 md:grid-cols-12 gap-6">
                    {block.items && block.items.map((item, i) => (
                        <div key={i} className={`col-span-1 md:col-span-${item.span} ${item.className || ''} bg-card rounded-3xl p-8 border border-border hover:border-primary/30 transition-colors shadow-lg`}>
                            <ContentRenderer blocks={item.content} compact={true} projectId={projectId} />
                        </div>
                    ))}
                </div>
            );
        ...
        <div className="bg-transparent min-h-screen">
            ...
            <div className="w-full px-4 sm:px-6 lg:px-12">
                <div className="max-w-5xl mx-auto">
                    ...
                    <article className={`pb-32 ${project.id === 'ui-designs' ? 'w-full' : ''}`}>
                        {project.content.map(chapter => (
                            <section key={chapter.number} className={project.id === 'ui-designs' ? 'mb-24' : ''}>
                                {project.id !== 'ui-designs' && <ChapterHeader chapter={chapter} />}
                                <div className={project.id === 'ui-designs' ? 'w-full' : 'max-w-4xl mx-auto'}>
                                    <ContentRenderer blocks={chapter.blocks} projectId={project.id} />
                                </div>
                            </section>
                        ))}
                    </article>
