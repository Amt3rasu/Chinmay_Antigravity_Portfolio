...
import React, { useMemo, useContext, createContext, useRef, useState } from 'react'
import { useFrame, useThree } from '@react-three/fiber'
import { getHighlightsData } from './highlightsData'
import { useMonitorStore, MonitorData } from '../../store/monitorStore'
import { useGLTF, Merged, RenderTexture, PerspectiveCamera, Text, useCursor } from '@react-three/drei'
...
export function Computers(props: any) {
    const { nodes: n, materials: m } = useGLTF('/models/computers_1-transformed.glb') as any
    const instances = useContext(context)

    // Get case study data for the 9 monitors
    const caseStudyScreens = useMemo(() => getHighlightsData(), [])
    ...
    return (
        <group {...props} dispose={null}>
            ...
            {/* Mapped Screens using Case Study Data with coordinated staggered entry */}
            <ScreenText caseStudy={caseStudyScreens[0]} frame="Object_206" panel="Object_207" position={[0.27, 1.53, -2.61]} staggerIndex={0} />
            <ScreenText caseStudy={caseStudyScreens[1]} frame="Object_209" panel="Object_210" y={5} position={[-1.43, 2.5, -1.8]} rotation={[0, 1, 0]} staggerIndex={2} />
            <ScreenText caseStudy={caseStudyScreens[2]} frame="Object_212" panel="Object_213" x={-5} y={5} position={[-2.73, 0.63, -0.52]} rotation={[0, 1.09, 0]} staggerIndex={4} />
            <ScreenText caseStudy={caseStudyScreens[3]} frame="Object_215" panel="Object_216" position={[1.84, 0.38, -1.77]} rotation={[0, -Math.PI / 9, 0]} staggerIndex={6} />
            <ScreenText caseStudy={caseStudyScreens[4]} frame="Object_218" panel="Object_219" x={-5} position={[3.11, 2.15, -0.18]} rotation={[0, -0.79, 0]} scale={0.81} staggerIndex={1} />
            <ScreenText caseStudy={caseStudyScreens[5]} frame="Object_221" panel="Object_222" y={5} position={[-3.42, 3.06, 1.3]} rotation={[0, 1.22, 0]} scale={0.9} staggerIndex={7} />
            <ScreenText caseStudy={caseStudyScreens[6]} frame="Object_224" panel="Object_225" position={[-3.9, 4.29, -2.64]} rotation={[0, 0.54, 0]} staggerIndex={8} />
            <ScreenText caseStudy={caseStudyScreens[7]} frame="Object_227" panel="Object_228" position={[0.96, 4.28, -4.2]} rotation={[0, -0.65, 0]} staggerIndex={5} />
            <ScreenText caseStudy={caseStudyScreens[8]} frame="Object_230" panel="Object_231" position={[4.68, 4.29, -1.56]} rotation={[0, -Math.PI / 3, 0]} staggerIndex={3} />
...
const Screen = React.forwardRef(({ frame, panel, children, ...props }: any, ref: any) => {
...
/* Renders a monitor with dynamic case study text */
function ScreenText({ caseStudy, frame, panel, x = 0, y = 1.2, staggerIndex = 0, ...props }: any) {
    const textRef = useRef<any>()

    // Hover interactions
    const setHoveredMonitor = useMonitorStore((state) => state.setHoveredMonitor)
    const setClickedMonitor = useMonitorStore((state) => state.setClickedMonitor)
    ...
    const getScreenPosition = () => {
        if (!groupRef.current) return { x: size.width / 2, y: size.height / 2 }
    ...
    const handlePointerOver = (e: any) => {
        ...
        setHovered(true)
        // Initial set
        setHoveredMonitor(caseStudy, getScreenPosition())
    }

    const handlePointerOut = (_e: any) => {
        hoverTimeout.current = setTimeout(() => {
            setHovered(false)
            setHoveredMonitor(null, undefined)
        }, 100)
    }

    const handleClick = (e: any) => {
        e.stopPropagation()
        setClickedMonitor(caseStudy)
    }

    useFrame((state) => {
        if (textRef.current) {
            // Calculations for coordinated rolling (Slowed and Densified)
            const cycleDuration = 20 // Total timeline
            const activeDuration = 10 // Increased from 8 to ensure ~4.5 active screens (4 or 5 always visible)
            const stagger = cycleDuration / 9

            // Current time in the cycle for THIS monitor
            const time = (state.clock.elapsedTime + staggerIndex * stagger) % cycleDuration

            if (time < activeDuration) {
                // Linear "Right-to-Left" roll
                const progress = time / activeDuration
                textRef.current.position.x = x + (12 - progress * 24)
            } else {
                // Keep off-screen during blank period
                textRef.current.position.x = x + 20
            }
        }
        // Continuous position update for pinned tooltip
        if (hovered) {
            setHoveredMonitor(caseStudy, getScreenPosition())
        }
    })
    ...
    // Determine colors
    const bgColor = caseStudy?.color || '#35c19f'
    ...
    return (
        <group>
            ...
            >
                ...
                {/* Internal brightness boost on hover */}
