...
const ContentRenderer: React.FC<{ blocks: ContentBlock[]; compact?: boolean }> = ({ blocks, compact = false }) => {
    ...
    const renderBlock = (block: ContentBlock, index: number) => {
        switch (block.type) {
            case 'h1':
                return <h1 key={index} className={`px-8 md:px-14 lg:px-20 max-w-4xl text-3xl md:text-4xl font-serif font-bold text-foreground ${compact ? 'mt-0 mb-2' : 'mt-16 mb-6'} tracking-tight`}>{block.content}</h1>;
            case 'h2':
                return <h2 key={index} className={`px-8 md:px-14 lg:px-20 max-w-4xl text-2xl font-bold text-foreground ${compact ? 'mt-0 mb-4' : 'mt-14 mb-4'}`}>{block.content}</h2>;
            case 'h3':
                return <h3 key={index} className={`px-8 md:px-14 lg:px-20 max-w-4xl text-xl font-bold text-foreground ${compact ? 'mt-0 mb-3' : 'mt-10 mb-3'}`}>{block.content}</h3>;
            case 'p':
            ...
            case 'aside':
                return (
                    <aside key={index} className={`mx-8 md:mx-14 lg:mx-20 max-w-3xl ${compact ? 'my-4' : 'my-10'} p-8 bg-card rounded-2xl border-l-4 border-primary shadow-lg`}>
            ...
            case 'split_layout':
                return (
                    <div key={index} className="px-8 md:px-14 lg:px-20 my-12 grid md:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            case 'bento_grid':
                return (
                    <div key={index} className="px-3 md:px-6 lg:px-8 my-16 grid grid-cols-1 md:grid-cols-12 gap-6">
                        {block.items?.map((item, i) => (
                            <div key={i} className={`col-span-1 md:col-span-${item.span} ${item.className || ''} bg-card rounded-3xl p-8 border border-border hover:border-primary/30 transition-colors shadow-lg`}>
                                <ContentRenderer blocks={item.content} compact />
                            </div>
                        ))}
                    </div>
                );
...
const ChapterHeader: React.FC<{ chapter: Chapter }> = ({ chapter }) => (
    <div className="relative my-24 px-8 md:px-14 lg:px-20">
        <div className="flex items-center gap-8">
            <div className="flex-1 border-t border-border" />
            <div className="px-8 text-center">
                <div className="text-5xl mb-3">{chapter.icon}</div>
                <h2 className="text-sm font-bold text-primary tracking-[0.2em] uppercase mb-2">Chapter {chapter.number}</h2>
                <p className="text-3xl font-serif font-bold text-foreground whitespace-nowrap">{chapter.title}</p>
            </div>
            <div className="flex-1 border-t border-border" />
        </div>
    </div>
                ...
                <article className="pb-32 w-full">
                    {project.content.map(chapter => (
                        <section key={chapter.number} className="mb-16">
                            {!isGallery && <ChapterHeader chapter={chapter} />}
